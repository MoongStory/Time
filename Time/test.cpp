...
int main()
{
	...
	std::cout << "get_current_time[" << MOONG::Time::get_current_time("YEAR년 month(format:02)월 day(format:02)일 HOUR(FORMAT:02)시 minute(foRMat:02)분 second(format:02)초 MILLISECONDS(format:03)밀리초") << "]" << std::endl;
	std::cout << "get_current_time[" << MOONG::Time::get_current_time("YEAR년 month(format:02)월 day(format:02)일(day_of_week요일) HOUR(FORMAT:02)시 minute(foRMat:02)분 second(format:02)초 MILLISECONDS(format:03)밀리초") << "]" << std::endl;
